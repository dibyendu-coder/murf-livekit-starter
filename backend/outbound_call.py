...
Usage:
    python outbound_call.py alice@sip.linphone.org
    python outbound_call.py +91XXXXXXXXXX          # phone number via PSTN trunk
    python outbound_call.py sip:alice@example.com  # explicit SIP URI
            ...
            try:
                sip_info = await lk.sip.create_sip_participant(
                    api.CreateSIPParticipantRequest(
                        ...
                        ringing_timeout      = api.google.protobuf.duration_pb2.Duration(
                            seconds=SIP_CALL_TIMEOUT
                        ) if hasattr(api, "google") else None,
                    )
                )
...
def make_outbound_call(destination: str) -> None:
    ...
    Args:
        destination: Where to call.  Accepted formats:
            "alice@sip.linphone.org"   — SIP URI (domain inferred from trunk if omitted)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "\nUsage:\n"
            "  python outbound_call.py <destination>\n\n"
            "Examples:\n"
            "  python outbound_call.py alice@sip.linphone.org\n"
            "  python outbound_call.py +91XXXXXXXXXX\n"
            "  python outbound_call.py sip:alice@sip.linphone.org\n\n"
